import sys
import time
from new_pid import PID


def test_zero(setpoint):
...
def test_converge_system(desired_heading):
    pid = PID(0.0083, 0.8, 0.04, output_limits=(-1.5, 1.5), sample_time=1)
    pid.setpoint = desired_heading

    def update_system(c, dt):  # simulates a change
    ...
    while time.time() - start_time < 120:
        pv = IMU.read_head_data()
        output = pid(pv)
        print(output, pv, pid.components)
        last_time = time.time()
        time.sleep(1)
...
def custom_test():
    [300]
    pid = PID(0.016,0.5,0,setpoint=0) #0.00833
    print(pid(30))
