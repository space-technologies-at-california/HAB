...
class HABVehicle:
    ...
    def checkStability(self):
        if (self.rockblock.geo[0] - 6376) < self.MIN_ALT:
            return False
        ...
        if self.imu.read_gyro_data()[2] > self.MAXDWN_ANGVEL:
            return False
        if self.imu.read_data().get('tb')[0] > self.MAX_PITCH:
            return False
        if self.imu.read_data().get('tb')[1] > self.MAX_ROLL:
            return False
        if self.imu.read_data().get('tb')[2] > self.MAX_YAW:
            return False
    
    def adjustStability(self):
        assigned_angle2 = self.servo2.duty
        assigned_angle1 = self.servo1.duty
        print("Servo Angles", assigned_angle1, assigned_angle2)
        while not self.checkStability():
            ...
            if self.imu.read_accel_data()[0] > self.MAXFWD_ACCEL:
                self.pid.reset(self.pid.kP, 0, 0, time.time())
            ...
            if self.imu.read_gyro_data()[1] > self.MAXLR_ANGVEL:
                self.pid.reset(self.pid.kP, 0, 0, time.time())
                self.pid.P_update(self.imu.read_gyro_data()[1], self.MAXLR_ANGVEL, self.MAXLR_ANGVEL, time.time())
                servo_adjustment = self.pid.PID_adjust(self.MAX_TURN)
                assigned_angle1 += min(servo_adjustment, self.MAX_TURN)
                assigned_angle2 -= min(servo_adjustment, self.MAX_TURN)
                self.servo2.setServo(assigned_angle2)
            ...
            if self.imu.read_gyro_data()[2] > self.MAXDWN_ANGVEL:
                self.pid.reset(self.pid.kP, 0, 0, time.time())
                self.pid.P_update(self.imu.read_gyro_data()[2], self.MAXDWN_ANGVEL, self.MAXDWN_ANGVEL, time.time())
                servo_adjustment = self.pid.PID_adjust(self.MAX_TURN)
                assigned_angle1 += min(servo_adjustment, self.MAX_TURN)
                assigned_angle2 -= min(servo_adjustment, self.MAX_TURN)
                self.servo2.setServo(assigned_angle2)
