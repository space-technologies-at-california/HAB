    ...
    double PascalToMeter(int32_t pressurePa)
        {
          ...
          const double P_b[N] = {102540.0, 22632.1, 5474.89, 868.02, 110.91};
          const double T_b[N] = {279.15, 216.65, 216.65, 228.65, 270.65};
...
private:
    ...
    uint16_t ReadCoefficient(const uint8_t coefNum)
    {
        digitalWrite(chipSelect, LOW);
        SPI.beginTransaction(settings);
        SPI.transfer(cmdPromRd_ + coefNum * 2);  // send PROM READ command
        uint8_t ret0 = SPI.transfer(0x00);
        uint8_t ret1 = SPI.transfer(0x00);
        uint16_t ret = ret1 | (ret0 << 8);
        SPI.endTransaction();
        Serial.println(ret);
        digitalWrite(chipSelect, HIGH);
        return ret;
...
//        Serial.print(pressAvg);        
    ...
    int32_t ReadAdc(const uint8_t cmd)
    {
        digitalWrite(chipSelect, LOW);
        SPI.beginTransaction(settings);
        SPI.transfer(cmdAdcConv_ | cmd);    // send conversion command
        SPI.endTransaction();
        digitalWrite(chipSelect, HIGH);
        ...
        digitalWrite(chipSelect, LOW);
        SPI.beginTransaction(settings);
        uint8_t ret0 = SPI.transfer(cmdAdcRead_);
        uint8_t ret1 = SPI.transfer(cmdAdcRead_);
        uint8_t ret2 = SPI.transfer(cmdAdcRead_);
        uint8_t ret3 = SPI.transfer(cmdAdcRead_);
//        uint32_t ret = SPI.transfer(cmdAdcRead_); // 24-bit ADC
        SPI.endTransaction();
        digitalWrite(chipSelect, HIGH);
        return (ret3 | (ret2 << 8) | (ret1 << 16)); //shifted 8-bits over (?)
